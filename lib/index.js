 ...
 * }} params
 */
async function setupDevExpress({ options, server }) {
  ...
  if (options?.entry) {
    ...
    console.info("Set the `entry` file for your server routes");
  }
  ...
  console.info("Express routes setup");
}
...
function vitePlugin(options) {
  return {
    ...
    //   return async () => await setupDevExpress({ options, server });
    // },
    transform(code, id) {
      if (options?.entry && id.includes("astro-express-server.js")) {
        const entry = entryToPath(options.entry);
        const outCode = `import _setupRouter from "${entry}";\n${code}`;
        return outCode;
      }
    },
  };
...
export default function integration(options) {
  ...
  return {
    ...
    hooks: {
      ...
      "astro:server:start": async function () {
        if (options?.entry) {
          const entry = entryToPath(options.entry);
